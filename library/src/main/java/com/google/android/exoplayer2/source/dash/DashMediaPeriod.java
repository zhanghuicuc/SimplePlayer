...
/* package */ final class DashMediaPeriod implements MediaPeriod,
  ...
  public DashMediaPeriod(int id, DashManifest manifest, int index,
    ...
    periodDurationUs = manifest.getPeriodDurationUs(index);
    trackGroups = buildTrackGroups(period, periodDurationUs);
  }
  ...
  private static TrackGroupArray buildTrackGroups(Period period, Long periodDurationUs) {
    TrackGroup[] trackGroupArray = new TrackGroup[period.adaptationSets.size()];
    for (int i = 0; i < period.adaptationSets.size(); i++) {
      AdaptationSet adaptationSet = period.adaptationSets.get(i);
      List<Representation> representations = adaptationSet.representations;
      Format[] formats = new Format[representations.size()];
      List<Float> QoEMedias = new ArrayList<>();
      List<Long> SegDurationsUs = new ArrayList<>();
      for (int j = 0; j < formats.length; j++) {
        formats[j] = representations.get(j).format;
        if( !representations.get(j).getQoeMedia().equals(Float.MIN_VALUE) ){
          QoEMedias.add(representations.get(j).getQoeMedia());
          SegDurationsUs.add(representations.get(j).getSegDurationUs(0, periodDurationUs));
        }
      }

      if (QoEMedias.size() == representations.size()) {
        trackGroupArray[i] = new TrackGroup(QoEMedias, SegDurationsUs, formats);
      } else {
